...
df = pd.concat([df, get_df_from_xls("marc_live.xlsm", "Live")], ignore_index=True)
df = pd.concat([df, get_df_from_xls("marc_yt.xlsm", "YouTube")], ignore_index=True)
...
filtered_df = (
    df[~df["songID"].str.startswith("cut")]
    .query("rank != 'I'")
    .query("htmlID != 's200704l'")
)
